...
population_data <- population_data[, c("nz_territory", "population")]

# Check for missing or duplicated values
population_data <- population_data[!is.na(population_data$population), ]
population_data <- population_data[!duplicated(population_data), ]

# Explore the distribution
hist(log(population_data$population), main = "Log-Transformed Population Distribution",
     xlab = "Log Population", col = "blue")
```

**Merge geospatial and numeric data**

```{r echo = FALSE}
# Task 7: Merge geospatial and population data
nz_map_data <- left_join(geojson_data, population_data, by = c("TA2016_NAM" = "nz_territory"))

```

**Make basic choropleth**
